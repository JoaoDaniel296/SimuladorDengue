...
class Verificacontinuação:
    ...
    def verificanumero(idade):
        if not re.fullmatch(r'-?\d+', idade):
            voltamenu = input("Digite apenas números inteiros! : (Pressione Enter para voltar ao cadastro do sobrevivente) ")
            Limpatela.clear_screen()
            menusimulacao()
        idade = int(idade)
        if idade < 0:
    ...
    def verificaporcentagem(idade):
        if not re.fullmatch(r'-?\d+(\.\d+)?', idade):
            voltamenu = input("Digite apenas números inteiros ou decimais (usando ponto)!: (Pressione Enter para voltar ao cadastro do sobrevivente) ")
            Limpatela.clear_screen()
            menusimulacao()
        idade = float(idade)
        if idade < 0:
...
def menusimulacao():
    ...
    Verificacontinuação.verificaporcentagem(multiplicamosquito)
...
def main():
    Limpatela.clear_screen()
    menu = str(input("Digite INICIAR se deseja iniciar uma nova simulação ou digite TABELA se deseja observar resultados de outras simulações: "))
